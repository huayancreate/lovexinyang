 ...
 */
(function($) {
	$.fn.loondialog = function(options) {
		var btn=new Array();
		if(options.btnsave){ btn.push("确定");}
		if(options.btnclose){ btn.push("取消");}
		var defaults = {
			title: '',
			content: '', //内容
			width:'', //宽度
			...
			otherButtons: btn, //其他按钮文本，样式默认,["确定","取消"]
		...
		return this.each(function() {
			...
			var tmpHtml ='<div class="modal fade" id="{ID}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel_{ID}" aria-hidden="true">'
					+'<div class="modal-dialog">'
						+'<div class="modal-content">'
							...
							+'</div>'
							+'<div class="modal-body" id="body_{ID}" style="overflow-y: auto;max-height: 600px; height:'+options.height+';">'
								+'{body}'
							+'</div>'
							+'<div class="modal-footer">'
								+'<img id="ajax-loader" style="display:none" height="32" width="32" src="' + loadimg + '" />'
								+'{button}'
							+'</div>'
						+'</div>'
					+'</div>'
	...
	$.extend({
		loondialog: function(options) {
			$("body").loondialog(options);
		},
	});
