...
public class RefundOrderDetailsActivity extends BaseActivity implements OnClickListener{
	...
	private void initView(){
		...
		lvCodes.setOnItemClickListener(new OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> adapter, View view, int position,long arg3) {
				// TODO 退款详情
				Codes  code=codeAdapter.getItem(position); 		
				Intent intent = new Intent(RefundOrderDetailsActivity.this, RefundDetailActivity.class);//退款详情
				intent.putExtra("goodsPwd", code.getGoodsPassword());
		...
		params.put("mobile", 1);
		 ...
		 HttpUtils.post(HttpUrl.ORDERACTION, params, new JsonHttpResponseHandler(){
			...
			public void onFailure(int statusCode, Header[] arg1, String responseString,Throwable error) {
					...
					try {
						JSONObject obj =  JSON.parseObject(GetData.getOrdersDetail(3));
						...
						if(result){
								...
								codeAdapter=new CodeAdapter(context, listCodes,2);
