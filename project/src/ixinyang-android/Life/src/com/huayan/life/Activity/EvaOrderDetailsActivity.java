...
public class EvaOrderDetailsActivity extends BaseActivity implements OnClickListener{
	...
	int detailsID;
	RatingBar rtb_eva;
	TextView tv_eva_order,tv_vali_date;
	...
	private void initView(){
		...
		((RelativeLayout)findViewById(R.id.rl_go_ping)).setOnClickListener(this);//去评价
		...
		rtb_eva=(RatingBar)findViewById(R.id.rtb_eva_or);
		tv_eva_order=(TextView)findViewById(R.id.tv_eva_or);
		...
		params.put("mobile", 1);
		 ...
		 HttpUtils.post(HttpUrl.ORDERACTION, params, new JsonHttpResponseHandler(){
			...
			public void onFailure(int statusCode, Header[] arg1, String responseString,Throwable error) {
					...
					try {
						JSONObject obj =  JSON.parseObject(GetData.getOrdersDetail(1));
						...
						if(result){
								...
								float commentScore=objContent.getFloat("commentScore");
								rtb_eva.setRating(commentScore);
								if(category.equals(getResources().getString(R.string.dai_eva))){
									tv_eva_order.setText(getResources().getString(R.string.go_eva));
								}else{
									tv_eva_order.setText(commentScore+"分");
								}
								...
								CodeAdapter codeAdapter=new CodeAdapter(context, listCodes,1);
